          ...
          if (category.sorts) {
            category.sorts.forEach(sort => {
              if (this.matchesSearch(sort, query)) {
                const url = `#/sorts-${this.slugify(category.nom)}`;
                console.log('Generated spell URL:', url, 'for category:', category.nom);
            ...
            category.dons.forEach(don => {
              if (this.matchesSearch(don, query)) {
                results.push({
                  ...
                  category: category.nom,
                  item: don,
                  url: `#/dons-${this.slugify(category.nom)}`,
