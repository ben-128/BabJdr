...
(() => {
  ...
  window.EventHandlers = {
    ...
    setupContentHandlers() {
      ...
      // Object references counter - mode MJ uniquement
      JdrApp.utils.events.register('click', '.object-references-counter', (e) => {
        e.preventDefault();
        e.stopPropagation();
        const objectNumero = e.target.dataset.objectNumero;
        if (objectNumero) {
          this.delegateToUI('showObjectReferencesModal', objectNumero);
        }
      });
    },
    ...
    delegateToUI(methodName, ...args) {
      ...
      } else if (methodName === 'showObjectReferencesModal') {
        // Delegate to ObjectReferencesModal
        if (window.ObjectReferencesModal && typeof window.ObjectReferencesModal.show === 'function') {
          return window.ObjectReferencesModal.show(...args);
        } else {
          console.warn('ObjectReferencesModal not available yet');
        }
      } else {
