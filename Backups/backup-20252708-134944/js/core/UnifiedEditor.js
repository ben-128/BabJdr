...
(() => {
  ...
  class UnifiedEditor {
    ...
    parseHtmlEditContext(element, editSection) {
      ...
      const pageDescSection = element.closest('.editable-section[data-section-type*="-category-description"]');
      if (pageDescSection) {
        ...
        return {
          ...
          pageType: contentType, // The actual content type (spell, don, objet, etc.)
          ...
          applyEdit: (content) => {
            ...
            if (success) {
              console.log(`✅ Updated page description for ${contentType}`);
            } else {
    ...
    updatePageDescriptionData(session, content) {
      ...
      if (success) {
        console.log(`✅ Updated page description for ${session.pageType}: "${content.substring(0, 50)}${content.length > 50 ? '...' : ''}"`);
        return true;
      } else {
