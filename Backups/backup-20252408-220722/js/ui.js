...
(() => {
  ...
  JdrApp.modules.ui = {
    ...
    createMonsterTagsModal(availableTags) {
      ...
      // Debug: check if delete buttons are created
      setTimeout(() => {
        const deleteButtons = modal.querySelectorAll('.delete-monster-tag-btn');
        console.log('Delete buttons found in modal:', deleteButtons.length);
        deleteButtons.forEach((btn, i) => {
          console.log(`Button ${i}:`, btn, 'Tag:', btn.dataset.tag, 'Visible:', btn.offsetWidth > 0);
        });
      }, 100);
    ...
    setupMonsterTagsModalEvents(modal, availableTags) {
      ...
      modal.addEventListener('click', (e) => {
        ...
        console.log('Modal click detected:', e.target, e.target.classList);
        
        if (e.target && e.target.classList.contains('delete-monster-tag-btn')) {
          const tagToDelete = e.target.dataset.tag;
          console.log('Delete button clicked for tag:', tagToDelete);
          ...
          if (index > -1) {
            ...
            if (confirm(`Êtes-vous sûr de vouloir supprimer le tag "${tagToDelete}" ?\n\nCela supprimera aussi ce tag de tous les monstres qui l'utilisent.`)) {
              ...
              // Regenerate monsters if currently viewing the page
