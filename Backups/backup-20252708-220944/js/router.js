...
(() => {
  ...
  JdrApp.modules.router = {
    ...
    renderCollectionsObjectsPage() {
      ...
      console.log('🔧 Collections page - Dev mode:', isDevMode);
      ...
      if (availableCollections) {
        if (isDevMode) {
          console.log('📋 Showing all collections in dev mode');
          ...
          console.log('🔍 Normal mode - collections hidden, search required');
          availableCollections.style.display = 'none';
          // Auto-search for "départ" in normal mode
          if (searchInput) {
            setTimeout(() => {
              this.displayCollection('départ');
            }, 100);
          }
        }
      }
    ...
    displayCollection(collectionName) {
      if (!window.OBJETS || !window.OBJETS.objets) return;
      
      // Filter objects by collection (tag)
      // In dev mode: partial match, in normal mode: exact match
      const isDevMode = JdrApp.utils.isDevMode();
      console.log('🔧 DEBUG displayCollection - isDevMode:', isDevMode, 'body classes:', document.body.className);
      const filteredObjects = window.OBJETS.objets.filter(obj => {
        if (!obj.tags) return false;
        
        if (isDevMode) {
          // Dev mode: partial match (as before)
          return obj.tags.some(tag => 
            tag.toLowerCase().includes(collectionName.toLowerCase())
          );
        } else {
          // Normal mode: exact match only
          return obj.tags.some(tag => 
            tag.toLowerCase() === collectionName.toLowerCase()
          );
        }
      });
      ...
      if (filteredObjects.length === 0) {
        ...
        // Clear search input after delay to show the error
        setTimeout(() => {
      ...
      // Update header (without back button)
      headerContainer.innerHTML = `
        <h2>📦 Collection: ${collectionName}</h2>
        <p>${filteredObjects.length} objet(s) trouvé(s)</p>
        ${isDevMode ? `
          <div class="collection-dev-actions" style="margin-top: 1rem;">
            <button class="btn small add-object-btn" data-collection="${collectionName}">➕ Ajouter un objet à cette collection</button>
          </div>
        ` : ''}
      `;
      
      // Get all unique tags from filtered objects
      const allTags = [...new Set(filteredObjects.flatMap(obj => obj.tags || []))];
      const tagsToShow = allTags.filter(tag => tag !== collectionName);
      ...
      // Setup add object button listener if in dev mode
      if (isDevMode) {
    ...
    renderCollectionItems(items) {
      ...
      console.log('🔍 DEBUG - Collection rendering:', { 
        isDevMode, 
        itemsCount: items.length,
        bodyClass: document.body.className,
        editorDevMode: JdrApp.modules?.editor?.isDevMode
      });
      ...
      if (!window.CardBuilder) {
        ...
        const itemsHTML = items.map(item => {
          ...
          if (isDevMode) {
            console.log('🔧 Adding dev controls for object:', item.numero);
      ...
      if (isDevMode) {
        console.log('🔧 Setting up collection edit controls');
        this.setupCollectionEditControls();
      } else {
        console.log('⚠️ Dev mode not detected, edit controls not added');
        console.log('To force dev mode, run: document.body.classList.add("dev-on")');
      }
    ...
    addObjectToCollection(collectionName) {
      ...
      const objectsList = allObjects
        .map((obj, index) => `${index + 1}. ${obj.nom} (N°${obj.numero})`)
        .join('\n');
      
      const selection = prompt(
        `Choisissez un objet à ajouter à la collection "${collectionName}":\n\n${objectsList}\n\nEntrez le numéro de l'objet (1-${allObjects.length}):`
      );
      
      if (!selection) return;
      
      const selectedIndex = parseInt(selection) - 1;
      if (selectedIndex < 0 || selectedIndex >= allObjects.length) {
      ...
      // Check if the object is already in this collection
      if (selectedObject.tags && selectedObject.tags.includes(collectionName)) {
        alert(`L'objet "${selectedObject.nom}" est déjà dans la collection "${collectionName}".`);
        return;
      }
      
      // Add the collection tag to the selected object
      if (!selectedObject.tags) {
        selectedObject.tags = [];
      }
      selectedObject.tags.push(collectionName);
      
      // Show notification
      if (JdrApp.modules.storage && JdrApp.modules.storage.showNotification) {
        JdrApp.modules.storage.showNotification(`➕ Objet "${selectedObject.nom}" ajouté à la collection "${collectionName}"`, 'success');
      }
      
      // Refresh the current collection view to show the added object
      this.displayCollection(collectionName);
    ...
    generateCollectionsGrid() {
      ...
      // Extract all unique tags from objects to create collections
      const tagCounts = {};
      ...
      const collectionsHTML = Object.entries(tagCounts)
        .sort(([a], [b]) => a.localeCompare(b))
        .map(([tagName, count]) => {
          const icon = collectionIcons[tagName] || '📦';
          return `
            <div class="collection-card" data-collection="${tagName}">
              <div class="collection-icon">${icon}</div>
              <h3>Collection ${tagName}</h3>
              <p>${count} objet(s)</p>
              ${isDevMode ? `
                <div class="collection-dev-controls">
                  <button class="btn small edit-collection-btn" data-collection="${tagName}" title="Renommer cette collection">✏️ Renommer</button>
                  <button class="btn small delete-collection-btn" data-collection="${tagName}" title="Supprimer cette collection">🗑️ Supprimer</button>
                </div>
              ` : ''}
            </div>
          `;
    ...
    createNewCollection() {
      ...
      window.OBJETS.objets.forEach(obj => {
        if (obj.tags) {
          obj.tags.forEach(tag => existingTags.add(tag));
        }
      });
      ...
      // Collection is created implicitly when objects are added to it
      // For now, we'll just refresh the grid and show a message
      if (JdrApp.modules.storage && JdrApp.modules.storage.showNotification) {
        JdrApp.modules.storage.showNotification(`✅ Collection "${trimmedName}" prête. Ajoutez des objets pour la rendre visible.`, 'success');
      }
    },

    renameCollection(oldName) {
      const newName = prompt(`Nouveau nom pour la collection "${oldName}":`, oldName);
      ...
      if (existingTags.has(trimmedNewName)) {
        alert(`La collection "${trimmedNewName}" existe déjà.`);
        return;
      }
      
      // Rename all instances of the old tag to the new tag
      let updatedCount = 0;
      window.OBJETS.objets.forEach(obj => {
        if (obj.tags) {
          const tagIndex = obj.tags.indexOf(oldName);
          if (tagIndex !== -1) {
            obj.tags[tagIndex] = trimmedNewName;
            updatedCount++;
          }
        }
      });
      ...
      if (updatedCount > 0) {
        ...
        if (JdrApp.modules.storage && JdrApp.modules.storage.showNotification) {
          JdrApp.modules.storage.showNotification(`✅ Collection "${oldName}" renommée en "${trimmedNewName}" (${updatedCount} objets mis à jour)`, 'success');
        }
      }
    },

    deleteCollection(collectionName) {
      const objectsWithTag = window.OBJETS.objets.filter(obj => 
        obj.tags && obj.tags.includes(collectionName)
      );
      
      if (objectsWithTag.length === 0) {
        alert(`La collection "${collectionName}" est déjà vide.`);
        return;
      }
      
      const confirmDelete = confirm(
        `Êtes-vous sûr de vouloir supprimer la collection "${collectionName}"?\n` +
        `Cela retirera ce tag de ${objectsWithTag.length} objet(s).\n` +
        `Les objets ne seront pas supprimés, seulement retirés de cette collection.`
      );
      ...
      window.OBJETS.objets.forEach(obj => {
        if (obj.tags) {
          const tagIndex = obj.tags.indexOf(collectionName);
          if (tagIndex !== -1) {
      ...
      if (JdrApp.modules.storage && JdrApp.modules.storage.showNotification) {
        JdrApp.modules.storage.showNotification(`🗑️ Collection "${collectionName}" supprimée (${updatedCount} objets mis à jour)`, 'success');
      }
    },
