  ...
  class UnifiedEditor {
    ...
    parseHtmlEditContext(element, editSection) {
      ...
      if (monsterCard) {
        ...
        // Extract field name from data-edit-section (format: "MonsterName-fieldName")
        const editSection = editableElement?.dataset?.editSection;
        let monsterEditSection = 'abilites'; // default
        
        if (editSection) {
          // Split by last hyphen to get the field name
          const parts = editSection.split('-');
          if (parts.length >= 2) {
            monsterEditSection = parts[parts.length - 1];
          }
        }
