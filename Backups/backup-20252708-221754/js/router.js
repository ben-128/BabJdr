...
(() => {
  ...
  JdrApp.modules.router = {
    ...
    displayCollection(collectionName) {
      ...
      // Create tag filters
      if (tagsToShow.length > 0 && tagFiltersContainer) {
        tagFiltersContainer.innerHTML = `
    ...
    generateCollectionsGrid() {
      if (!window.OBJETS || !window.OBJETS.objets) {
        console.error('❌ No OBJETS data available');
        return;
      }
      ...
      console.log('🏷️ Found collections:', tagCounts);
      ...
      console.log('📦 Collections grid generated:', {
        collectionsCount: Object.keys(tagCounts).length,
        ...
        this.devModeObserver = new MutationObserver((mutations) => {
          mutations.forEach((mutation) => {
            if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
              const hasDevOn = document.body.classList.contains('dev-on');
              const hasDevOff = document.body.classList.contains('dev-off');
              
              if (hasDevOn || hasDevOff) {
                console.log('🔄 Body class changed, refreshing collections view');
    ...
    refreshCollectionsView() {
      // Only refresh if we're on the collections page
      const currentHash = window.location.hash;
