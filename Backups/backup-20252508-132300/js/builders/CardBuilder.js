...
// console.warn('Found array format for capacites, converting to HTML:', items);
    ...
    buildMonsterCard() {
      ...
      const tagsHtml = this.data.tags ? this.data.tags.map(tag => `<span class="tag">${tag}</span>`).join(' ') : '';
      ...
      return `
        <div class="card editable-section" data-section-type="monster" data-monster-name="${this.data.nom}" data-category-name="${this.categoryName}">
          ...
          <div style="display: flex; justify-content: space-between; align-items: center; margin: 1rem 0;">
            <div class="monster-tags" style="flex: 1;">
              ${this.buildEditableTagsField(tagsHtml, 'monster-tags', 'Tags')}
            </div>
            <div class="monster-element" style="margin-left: 1rem;">
              ${this.shouldShowEditButtons ? 
                `<div class="editable-section" data-section-type="monster-element">
                   <div class="editable" data-edit-type="select" data-edit-section="${this.data.nom}-monster-element" 
          ...
          ${this.data.image ? `
          <div class="monster-image" style="text-align: center; margin: 1rem 0;">
            ${this.shouldShowEditButtons ? 
              `<div class="editable-section" data-section-type="monster-image">
                 <img src="${this.data.image}" alt="${this.data.nom}" style="max-width: 200px; height: auto; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);" class="editable" data-edit-type="image" data-edit-section="${this.data.nom}-monster-image">
                 ${this.buildEditButton('image')}
               </div>` : 
              `<img src="${this.data.image}" alt="${this.data.nom}" style="max-width: 200px; height: auto; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">`
            }
          </div>
          ` : ''}

          <!-- Stats Section -->
          <div class="monster-stats" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 0.5rem; margin: 1rem 0; padding: 1rem; background: var(--card); border-radius: 8px;">
            <div><strong>PV:</strong> ${this.data.pointsDeVie}</div>
            <div><strong>Armure:</strong> ${this.data.armurePhysique}</div>
            <div><strong>Esquive:</strong> ${this.data.esquive}</div>
            <div><strong>Crit:</strong> ${this.data.coupCritique}</div>
            <div><strong>Crit sorts:</strong> ${this.data.coupCritiqueSorts}</div>
            <div><strong>Résist:</strong> ${this.data.resistanceAlterations}</div>
          </div>

          <!-- Elemental Armor Section -->
          <div class="monster-armor" style="margin: 1rem 0;">
            <h4 style="margin: 0 0 0.5rem 0; color: var(--bronze);">🛡️ Armures élémentaires</h4>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(90px, 1fr)); gap: 0.5rem; font-size: 0.9em;">
              <div style="display: flex; align-items: center;">🔥 <strong style="margin-left: 4px;">${this.data.armureFeu || 0}</strong></div>
              <div style="display: flex; align-items: center;">💧 <strong style="margin-left: 4px;">${this.data.armureEau || 0}</strong></div>
              <div style="display: flex; align-items: center;">🤎 <strong style="margin-left: 4px;">${this.data.armureTerre || 0}</strong></div>
              <div style="display: flex; align-items: center;">🟢 <strong style="margin-left: 4px;">${this.data.armureAir || 0}</strong></div>
              <div style="display: flex; align-items: center;">☀️ <strong style="margin-left: 4px;">${this.data.armureLumiere || 0}</strong></div>
              <div style="display: flex; align-items: center;">⚫ <strong style="margin-left: 4px;">${this.data.armureObscurite || 0}</strong></div>
              <div style="display: flex; align-items: center;">⚪ <strong style="margin-left: 4px;">${this.data.armureDivin || 0}</strong></div>
              <div style="display: flex; align-items: center;">🟣 <strong style="margin-left: 4px;">${this.data.armureMalefique || 0}</strong></div>
            </div>
          </div>
