...
(() => {
  ...
  class PageBuilder {
      ...
      return `
        <article class="" data-page="${pageId}" data-page-title="${classData.nom}">
          <section>
            ...
            ${this.buildEditableList(classData.capacites, 'class-capacites', classData.nom)}
            <h3>Sous-classes</h3>
    ...
    buildEditableList(items, editType, editSection) {
      // Convert to HTML and use simple HTML rendering
    ...
    buildGridSection(gridData) {
      // Convert grid to simple HTML
      const gridContent = gridData.content ? gridData.content.map(item => this.buildCardSection(item)).join('') : 
                         gridData.items ? gridData.items.map(item => this.buildCardSection(item)).join('') : '';
      ...
      return `
        <div class="editable-section" data-section-type="html">
          <div class="editable" data-edit-type="html" data-edit-section="grid">
            ${htmlContent}
          </div>
          ${this.buildEditButton('section')}
        </div>
