    ...
    parseHtmlEditContext(element, editSection) {
      ...
      if (classArticle) {
        ...
        if (subclassCard) {
          ...
          if (editableElement) {
            ...
            if (editableElement.dataset.editSection) {
              subclassEditSection = editableElement.dataset.editSection;
            } else if (parentSection?.dataset.editSection) {
              subclassEditSection = parentSection.dataset.editSection;
            } else if (editableElement.dataset.editType) {
              subclassEditSection = editableElement.dataset.editType;
            } else {
              // Fallback: analyze content and context
              const content = editableElement.textContent || editableElement.innerHTML;
              if (element.closest('.stats-grid')) {
                subclassEditSection = 'base';
              } else if (content.includes('Progression') || content.includes('niveau')) {
                subclassEditSection = 'progression';  
              } else if (editableElement.tagName === 'UL' || content.includes('Capacité')) {
        ...
        if (editableElement) {
          ...
          // Check data attributes first
          if (editableElement.dataset.editSection) {
            classEditSection = editableElement.dataset.editSection;
          } else if (parentSection?.dataset.editSection) {
            classEditSection = parentSection.dataset.editSection;
          } else if (editableElement.dataset.editType) {
            classEditSection = editableElement.dataset.editType;
          } else {
            // Fallback: analyze content and context
            const content = editableElement.textContent || editableElement.innerHTML;
            if (content.includes('Résumé') || content.includes('combat au corp')) {
              classEditSection = 'resume';
            } else if (editableElement.tagName === 'UL' || content.includes('Expert de') || content.includes('Capacité')) {
