  ...
  class UnifiedEditor {
    ...
    parseHtmlEditContext(element, editSection) {
      ...
      if (editableSection) {
        ...
        // Determine the category type (spell/don/table-tresor/monster/etc) and category name
        let contentType = 'spell'; // default
        if (sectionType.includes('spell')) {
          contentType = 'spell';
        } else if (sectionType.includes('don')) {
          contentType = 'don';
        } else if (sectionType.includes('table-tresor')) {
          // Special handling for table tresor page description - save directly to global
          return {
            contentType: 'specialPage',
            itemIdentifier: 'tables-tresors',
            categoryName: null,
            property: 'description',
            editType: 'html',
            editSection: 'description',
            globalVar: 'TABLES_TRESORS_PAGE_DESC',
            element: editableElement,
            container: element,
            applyEdit: (content) => {
              ...
              console.log('✅ Updated TABLES_TRESORS_PAGE_DESC:', window.TABLES_TRESORS_PAGE_DESC);
            }
          };
        } else if (sectionType.includes('monster')) {
          // Special handling for monster page description - save directly to global
          return {
            contentType: 'specialPage',
            itemIdentifier: 'monstres', 
            categoryName: null,
            property: 'description',
            editType: 'html',
            editSection: 'description',
            globalVar: 'MONSTRES_PAGE_DESC',
            element: editableElement,
            container: element,
            applyEdit: (content) => {
              ...
              console.log('✅ Updated MONSTRES_PAGE_DESC:', window.MONSTRES_PAGE_DESC);
            }
          };
        }
    ...
    updateTableTresorData(session, content) {
      ...
      try {
        ...
        if (propertyName === 'nom' && this.currentEditSession && this.currentEditSession.itemIdentifier === originalValue) {
          console.log('🔄 Updating session itemIdentifier from', this.currentEditSession.itemIdentifier, 'to', content);
        ...
        console.log('✅ Table tresor update successful:', table);
        return true;
