  ...
  class UnifiedEditor {
    ...
    saveToJson(session, content, jsonCategory) {
      ...
      switch (session.contentType) {
        case 'spell':
        case 'don':
          return this.saveItemToJson(jsonData, session, content);
          
        case 'class':
          return this.saveClassToJson(jsonData, session, content);
          
        case 'subclass':
          return this.saveSubclassToJson(jsonData, session, content);
          
        case 'staticPage':
          return this.saveStaticPageToJson(jsonData, session, content);
          
        default:
          console.warn('Unsupported content type for JSON save:', session.contentType);
          return false;
      }
    }

    saveItemToJson(jsonData, session, content) {
      // Find the category and item within the JSON structure
      for (const category of jsonData) {
        if (category.nom === session.categoryName) {
          const itemsKey = session.contentType === 'spell' ? 'sorts' : 'dons';
          const item = category[itemsKey]?.find(item => item.nom === session.itemIdentifier);
          if (item) {
            // Save HTML content directly to the item
            item[session.editSection || 'content'] = content;
            return true;
          }
        }
      }
    ...
    saveClassToJson(jsonData, session, content) {
      const classe = jsonData.find(c => c.nom === session.itemIdentifier);
      if (classe) {
        // Map the editSection to the actual property in the class JSON structure
        const propertyMap = {
          'resume': 'resume',
          'capacites': 'capacites'
        };
        const property = propertyMap[session.editSection] || session.editSection;
        classe[property] = content;
        return true;
      }
      return false;
    }

    saveSubclassToJson(jsonData, session, content) {
      const classe = jsonData.find(c => c.nom === session.categoryName);
      if (classe?.sousClasses) {
        const sousClasse = classe.sousClasses.find(sc => sc.nom === session.itemIdentifier);
        if (sousClasse) {
          // Map the editSection to the actual property in the subclass JSON structure
          const propertyMap = {
            'progression': 'progression',
            'capacites': 'capacites'
          };
          const property = propertyMap[session.editSection] || session.editSection;
          sousClasse[property] = content;
          return true;
        }
      }
      return false;
