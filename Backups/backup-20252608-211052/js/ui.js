...
(() => {
  ...
  JdrApp.modules.ui = {
      ...
      const spellsHTML = spells.map(spell => `
        <div class="spell-item" data-spell-name="${spell.name}" data-spell-category="${spell.category}">
          <div class="spell-info">
            ...
            <div class="spell-meta">${spell.element} • ${spell.category}</div>
    ...
    getElementColor(element) {
      const colors = window.ElementColors;
      const baseColor = colors[element]?.color || '#666';
      
      // Améliorer le contraste pour certaines couleurs problématiques
      const colorMap = {
        '#ffd700': '#b8860b', // Lumière: plus foncé pour la lisibilité
        '#f5f5f5': '#555555', // Divin: beaucoup plus foncé
        '#1a1a1a': '#444444'  // Nuit: un peu plus clair
      };
      ...
      popup.innerHTML = `
        <div class="spell-preview-content">
          <div class="spell-preview-header" style="border-left: 4px solid ${elementColor};">
            ...
            <div class="spell-element">${spellData.element} • ${categoryName}</div>
          ...
          <div class="spell-preview-body">
            ...
            ${spellData.effetNormal ? `<div class="spell-field"><strong>Effet:</strong> ${spellData.effetNormal}</div>` : ''}
            ${spellData.effetCritique ? `<div class="spell-field"><strong>Critique:</strong> ${spellData.effetCritique}</div>` : ''}
          </div>
        </div>
      `;
