      ...
      const updateMobileAnimations = () => {
        ...
        document.querySelectorAll('.illus img').forEach(img => {
          ...
          if (rect.top < windowHeight && rect.bottom > 0) {
            ...
            // Enhanced effects when near center
            if (Math.abs(distanceFromCenter) < 100) {
              img.style.setProperty('filter', 'brightness(1.25) contrast(1.25)', 'important');
              img.style.setProperty('box-shadow', '0 25px 50px rgba(0,0,0,0.4), 0 0 40px rgba(212,175,55,0.7)', 'important');
            } else {
              img.style.setProperty('filter', 'brightness(1.1) contrast(1.1)', 'important');
              img.style.setProperty('box-shadow', '0 15px 30px rgba(0,0,0,0.3), 0 0 20px rgba(212,175,55,0.4)', 'important');
            }
          } else {
