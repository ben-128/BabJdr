...
(() => {
  ...
  window.ModalManager = {
    ...
    showCartesDestinModal() {
      console.log('🎴 showCartesDestinModal() appelée');
      console.log('JdrApp.utils.dom existe?', typeof JdrApp.utils.dom);
      console.log('JdrApp.utils.dom.$ existe?', typeof JdrApp.utils.dom?.$);

      let modal = JdrApp.utils.dom.$('#cartesDestinModal');
      console.log('Modal existant trouvé?', !!modal);
      ...
      if (!modal) {
        ...
        document.body.appendChild(modal);
        console.log('Modal ajouté au body');
      }

      console.log('BaseModal existe?', typeof BaseModal);
      console.log('BaseModal.openModal existe?', typeof BaseModal?.openModal);

      if (BaseModal && typeof BaseModal.openModal === 'function') {
        console.log('Appel de BaseModal.openModal()');
        BaseModal.openModal(modal);
        console.log('BaseModal.openModal() terminé');
      } else {
        console.error('❌ BaseModal ou BaseModal.openModal non disponible');
      }
    },
    ...
    createCartesDestinModal() {
      console.log('📝 createCartesDestinModal() appelée');
      console.log('window.STATIC_PAGES existe?', !!window.STATIC_PAGES);
      console.log('window.STATIC_PAGES.creation existe?', !!window.STATIC_PAGES?.creation);

      // Récupérer les données depuis le JSON de la page création
      const creationData = window.STATIC_PAGES?.creation?.cartesDestin;
      console.log('creationData trouvé?', !!creationData);
      console.log('creationData:', creationData);

      if (!creationData) {
        console.error('Données des cartes du destin non trouvées');
        console.error('window.STATIC_PAGES.creation:', window.STATIC_PAGES?.creation);
        return null;
      }
      ...
      console.log('Modal créé par BaseModal.createModal:', modal);
