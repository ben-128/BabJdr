  ...
  class UnifiedEditor {
    ...
    parseHtmlEditContext(element, editSection) {
      ...
      if (staticPageArticle) {
        ...
        console.log('Static page structure analysis:', {
          pageName,
          currentIndex,
          totalEditables: allEditables.length
        });
        
        // For static pages, use the editType from the closest editable-section or card
        const cardOrSection = element.closest('.card') || element.closest('[data-edit-type]');
        const staticEditSection = cardOrSection?.dataset.editType || editSection || `section-${currentIndex}`;
        
        return {
          contentType: 'staticPage',
          itemIdentifier: pageName,
          categoryName: null,
          property: 'html',
          editType: 'html',
          editSection: staticEditSection,
      ...
      if (spellCard) {
        ...
        console.log('Spell structure analysis:', {
          spellName,
          currentIndex,
          totalEditables: allEditables.length
        });
      ...
      if (donCard) {
        ...
        console.log('Don structure analysis:', {
          donName,
          currentIndex,
          totalEditables: allEditables.length
        });
      ...
      if (classArticle) {
        ...
        if (subclassCard) {
          ...
          if (editableElement) {
            ...
            console.log('Subclass structure analysis:', {
              currentIndex,
              totalEditables: allEditables.length,
              elementTag: editableElement.tagName,
              elementClasses: editableElement.className,
              inStatsGrid: !!element.closest('.stats-grid')
            });
            
            // Map index to property based on subclass structure  
            // Index 0 = base (stats section)
            // Index 1 = progression (progression section)  
            // Index 2+ = capacites (capabilities section)
            if (element.closest('.stats-grid') || currentIndex === 0) {
