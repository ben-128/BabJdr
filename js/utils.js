...
(() => {
  ...
  JdrApp.utils.dom = {
    ...
    createCard(className = '', content = '') {
      return this.create('div', `card ${className}`, content);
    },
    
    // Chip creation pattern
    createChip(content, className = '') {
      return this.create('div', `chip ${className}`, content);
    },
    
    // Button creation pattern
    createButton(text, className = 'btn', attributes = {}) {
      return this.create('button', className, text, attributes);
    },
    ...
    createContainer(className = '', children = []) {
      const container = this.create('div', className);
    ...
    // Common patterns for app
    createIllustration(illusKey, altText = '') {
      return this.create('div', 'illus', `
        <img class="thumb" alt="${altText}" style="display: none;">
        <label class="up">📷 Ajouter<input type="file" accept="image/*" hidden=""></label>
        <button class="rm" type="button" style="display: none;">🗑 Retirer</button>
      `, {
        'data-illus-key': illusKey,
        'data-bound': '1'
      });
    },
  ...
  JdrApp.utils.compressImage = function(file, maxWidth = 800, quality = 0.85) {
    return new Promise((resolve, reject) => {
      console.log(`🖼️ Compressing image: ${file.name}, original size: ${Math.round(file.size/1024)}KB`);
      ...
      img.onload = function() {
        ...
        console.log(`🖼️ Original dimensions: ${width}x${height}`);
        ...
        if (isPNG) {
          ...
          console.log('🖼️ PNG detected, using original file without compression');
          resolve(file);
        } else {
          // For JPEG/other formats, use white background
          console.log('🖼️ JPEG format, using white background');
          ctx.fillStyle = '#FFFFFF';
          ctx.fillRect(0, 0, width, height);
          ctx.drawImage(img, 0, 0, width, height);
          
          canvas.toBlob((blob) => {
            console.log(`🖼️ JPEG compressed: ${Math.round(blob.size/1024)}KB, ${width}x${height}, quality: ${quality}`);
            resolve(blob);
          }, 'image/jpeg', quality);
        }
      };
    ...
    return new Promise((resolve, reject) => {
      ...
      .then(data => {
        ...
        if (data.success) {
          ...
          console.log('🖼️ Image info:', `${data.data.width}x${data.data.height}, ${Math.round(data.data.size/1024)}KB`);
