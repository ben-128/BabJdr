  ...
  JdrApp.modules.ui = {
    ...
    setupContentHandlers() {
      ...
      JdrApp.utils.events.register('click', '.move-fourchette-up-btn', (e) => {
        ...
        if (tableName && fourchetteIndex > 0) {
          this.moveFourchette(tableName, fourchetteIndex, fourchetteIndex - 1);
        }
      });
    ...
    setupTableTresorTagsModalEvents(modal, availableTags) {
      ...
      const addTag = () => {
        ...
        if (newTag && !availableTags.includes(newTag)) {
          ...
          // Force refresh of the tables-tresors page to show new tag in filters
    ...
    showMonsterLinksModal() {
      ...
      this.openModal('monsterLinksModal');
    },

    createMonsterLinksModal() {
      // Récupérer tous les monstres depuis window.MONSTRES
      const monsters = [];
      ...
      if (window.MONSTRES && Array.isArray(window.MONSTRES)) {
        window.MONSTRES.forEach(monster => {
          monsters.push({
            ...
            pointsDeVie: monster.pointsDeVie || 0,
            tags: monster.tags || [],
            description: monster.abilites || '',
            image: monster.image || ''
          });
        });
      }
      ...
      const monstersHTML = monsters.map(monster => `
        <div class="monster-item" data-monster-name="${monster.name}">
          <div class="monster-info">
            <div class="monster-name" style="color: ${this.getElementColor(monster.element)}; text-shadow: 0 1px 2px rgba(0,0,0,0.3);">${monster.name}</div>
            <span data-monster-meta style="color: ${this.getElementColor(monster.element)} !important; font-size: 12px; margin-bottom: 6px; display: block;">${this.getElementIcon(monster.element)} ${monster.element} • ${monster.pointsDeVie} PV • ${monster.tags.join(', ')}</span>
            <div class="monster-description">${this.stripHtml(monster.description).length > 80 ? this.stripHtml(monster.description).substring(0, 80) + '...' : this.stripHtml(monster.description)}</div>
          </div>
          <div class="copy-indicator">Copié!</div>
        </div>
      `).join('');

      const modal = JdrApp.utils.dom.create('div', 'modal monster-links-modal', `
        <div class="modal-content monster-links-modal-content">
          <div style="position: relative;">
            <button class="modal-close-x" style="position: absolute; top: 0; right: 0; background: none; border: none; font-size: 20px; cursor: pointer; color: var(--paper-muted); padding: 4px 8px; border-radius: 4px; transition: all 0.2s ease;" onmouseover="this.style.background='var(--rule)'; this.style.color='var(--accent-ink)';" onmouseout="this.style.background='none'; this.style.color='var(--paper-muted)';">×</button>
            <h3>🐲 Liens vers les monstres</h3>
          </div>
          <p>Cliquez sur un monstre pour copier un lien avec aperçu interactif.</p>
          
          <div style="margin-bottom: 1rem;">
            <input type="text" id="monsterSearchInput" placeholder="Rechercher un monstre..." style="width: 100%; padding: 8px; border: 1px solid var(--rule); border-radius: 4px; background: var(--paper); color: var(--paper-ink);">
          </div>
          
          <div class="monsters-list" style="max-height: 400px; overflow-y: auto;">
            ${monstersHTML}
          </div>
        </div>
      `);
      ...
      const searchInput = modal.querySelector('#monsterSearchInput');
      if (searchInput) {
        searchInput.addEventListener('input', (e) => {
          const searchTerm = e.target.value.toLowerCase();
          const monsterItems = modal.querySelectorAll('.monster-item');
          
          monsterItems.forEach(item => {
            const monsterName = item.querySelector('.monster-name').textContent.toLowerCase();
            const monsterMeta = item.querySelector('[data-monster-meta]').textContent.toLowerCase();
            const monsterDesc = item.querySelector('.monster-description').textContent.toLowerCase();
            
            const matches = monsterName.includes(searchTerm) || 
                          monsterMeta.includes(searchTerm) || 
                          monsterDesc.includes(searchTerm);
      ...
      modal.addEventListener('click', (e) => {
        ...
        if (monsterItem) {
          ...
          this.copyToClipboard(monsterLink);
    ...
    showMonsterPreview(monsterName, triggerElement, event) {
      // Trouver le monstre dans les données
      let monsterData = null;
      
      if (window.MONSTRES && Array.isArray(window.MONSTRES)) {
        monsterData = window.MONSTRES.find(monster => monster.nom === monsterName);
      }

      if (!monsterData) {
        this.showNotification(`❌ Monstre "${monsterName}" non trouvé`, 'error');
        return;
      }

      // Fermer toute preview existante
      const existingPreview = document.querySelector('.monster-preview-popup');
      if (existingPreview) {
        existingPreview.remove();
      }

      // Calculer la position de la popup à partir de la souris si disponible
      const viewportWidth = window.innerWidth;
      const viewportHeight = window.innerHeight;
      
      // Utiliser la position de la souris si disponible, sinon fallback sur l'élément
      const mouseX = event ? event.clientX : triggerElement.getBoundingClientRect().left;
      const mouseY = event ? event.clientY : triggerElement.getBoundingClientRect().bottom;
      
      // Créer la popup de preview
      const popup = document.createElement('div');
      popup.className = 'monster-preview-popup';
      
      const elementColor = this.getElementColor(monsterData.element);
      
      // Construire les résistances élémentaires avec les bonnes icônes
      const resistances = [];
      const armures = {
        'Feu': { value: monsterData.armureFeu, icon: '🔥' },
        'Eau': { value: monsterData.armureEau, icon: '💧' },
        'Terre': { value: monsterData.armureTerre, icon: '⛰️' },
        'Air': { value: monsterData.armureAir, icon: '💨' },
        'Lumière': { value: monsterData.armureLumiere, icon: '✨' },
        'Obscurité': { value: monsterData.armureObscurite, icon: '🌑' },
        'Divin': { value: monsterData.armureDivin, icon: '✨' },
        'Maléfique': { value: monsterData.armureMalefique, icon: '👹' }
      };
      
      for (const [element, data] of Object.entries(armures)) {
        if (data.value && data.value > 0) {
          resistances.push(`${data.icon} ${element}: ${data.value}`);
        }
      }

      popup.innerHTML = `
        <div class="monster-preview-content">
          <div class="monster-preview-header" style="border-left: 4px solid ${elementColor};">
            <div class="monster-title" style="color: ${elementColor}; text-shadow: 0 1px 2px rgba(0,0,0,0.3);">${monsterData.nom}</div>
            <span data-element-display style="color: ${elementColor} !important; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px;">${this.getElementIcon(monsterData.element)} ${monsterData.element}</span>
            <button class="monster-preview-close">✕</button>
          </div>
          <div class="monster-preview-body">
            <div class="monster-stats" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px; margin-bottom: 12px; font-size: 12px;">
              <div><strong>❤️ PV:</strong> ${monsterData.pointsDeVie}</div>
              <div><strong>🛡️ Armure:</strong> ${monsterData.armurePhysique || 0}</div>
              <div><strong>💨 Esquive:</strong> ${monsterData.esquive || 0}</div>
              <div><strong>⚡ Initiative:</strong> ${monsterData.initiative || 0}</div>
              <div><strong>💥 Critique:</strong> ${monsterData.coupCritique || 0}</div>
              <div><strong>🎯 Crit. Sorts:</strong> ${monsterData.coupCritiqueSorts || 0}</div>
              <div><strong>🛡️ Résist. Alt.:</strong> ${monsterData.resistanceAlterations || 0}</div>
            </div>
            ${resistances.length > 0 ? `<div class="monster-resistances" style="margin-bottom: 12px; font-size: 12px;"><strong>Résistances élémentaires:</strong><br>${resistances.join(', ')}</div>` : ''}
            ${monsterData.abilites ? `<div class="monster-abilities" style="margin-top: 12px;"><strong>Capacités:</strong><div style="margin-top: 6px;">${monsterData.abilites}</div></div>` : ''}
            ${monsterData.butin ? `<div class="monster-loot" style="margin-top: 12px;"><div style="margin-top: 6px;">${monsterData.butin}</div></div>` : ''}
            <div class="monster-actions" style="margin-top: 16px; padding-top: 12px; border-top: 1px solid var(--rule); text-align: center;">
              <button class="monster-goto-page" style="background: var(--accent); color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-size: 12px;">
                🔍 Voir dans la page Monstres
              </button>
            </div>
          </div>
        </div>
      ...
      // Calculer position après ajout au DOM pour obtenir les dimensions réelles
      const popupRect = popup.getBoundingClientRect();
      const popupWidth = popupRect.width;
      const popupHeight = popupRect.height;

      // Position de base : décalage à partir de la position de la souris
      // mouseX et mouseY sont déjà en coordonnées viewport, on ajoute le scroll pour les coordonnées absolues
      let left = mouseX + 15; // 15px à droite de la souris
      let top = mouseY + 10;   // 10px en bas de la souris

      // Ajuster si la popup déborde horizontalement
      if (left + popupWidth > viewportWidth) {
        left = mouseX - popupWidth - 15; // 15px à gauche de la souris
      }
      if (left < 10) {
        left = 10; // Marge minimale du bord gauche
      }

      // Ajuster si la popup déborde verticalement
      if (top + popupHeight > viewportHeight) {
        top = mouseY - popupHeight - 10; // 10px au-dessus de la souris
      }
      if (top < 10) {
        top = 10; // Marge minimale du bord haut
      }

      // Convertir en coordonnées absolues en ajoutant le scroll
      popup.style.left = `${left + window.scrollX}px`;
      popup.style.top = `${top + window.scrollY}px`;

      // Gestionnaires d'événements
      popup.querySelector('.monster-preview-close').addEventListener('click', () => {
        popup.remove();
      });

      // Bouton pour aller à la page monstres
      popup.querySelector('.monster-goto-page').addEventListener('click', () => {
        popup.remove();
        this.goToMonstersPage(monsterName);
      });
    ...
    goToMonstersPage(monsterName) {
      // Naviguer vers la page monstres
      if (JdrApp.modules.router) {
        JdrApp.modules.router.navigate('monstres');
        
        // Attendre que la page soit chargée puis filtrer le monstre
        setTimeout(() => {
          this.filterMonsterByName(monsterName);
        }, 300);
      }
    },

    filterMonsterByName(monsterName) {
      // Trouver la carte du monstre et la mettre en évidence
      setTimeout(() => {
        const monsterCards = document.querySelectorAll('[data-monster-name]');
        let targetCard = null;
        
        monsterCards.forEach(card => {
          const cardMonsterName = card.dataset.monsterName || 
                                  card.querySelector('h3, .card-title, .monster-name')?.textContent?.trim();
          
          if (cardMonsterName === monsterName) {
            targetCard = card;
          }
        });
        
        if (targetCard) {
          // Scroller vers le monstre
          ...
          targetCard.style.boxShadow = '0 0 20px rgba(139, 92, 23, 0.5)';
          ...
          setTimeout(() => {
            ...
            // Attendre la transition avant de supprimer
            setTimeout(() => {
          ...
          this.showNotification(`🎯 Monstre "${monsterName}" mis en évidence`, 'success');
        } else {
          this.showNotification(`❌ Monstre "${monsterName}" non trouvé dans la page`, 'error');
        }
      }, 100);
    },
    ...
    // Déplacer une fourchette
    moveFourchette(tableName, fromIndex, toIndex) {
      const table = this.findTableTresor(tableName);
      if (!table || !table.fourchettes || fromIndex < 0 || toIndex < 0 || 
          fromIndex >= table.fourchettes.length || toIndex >= table.fourchettes.length) {
        this.showNotification('Impossible de déplacer la fourchette', 'error');
        return;
      }

      // Échanger les fourchettes
      const temp = table.fourchettes[fromIndex];
      table.fourchettes[fromIndex] = table.fourchettes[toIndex];
      table.fourchettes[toIndex] = temp;
      ...
      // Rechercher et rerendre la carte de la table
      const tableContainer = tableCard.closest('.grid');
      if (tableContainer) {
        // Trouver l'index de la table dans le DOM pour la reconstruire
        const allTables = Array.from(tableContainer.children);
        const tableIndex = allTables.indexOf(tableCard);
        
        if (tableIndex >= 0) {
          // Reconstruire la carte avec les nouvelles données
          const newCard = CardBuilder.create('tableTresor', table, 'tables', tableIndex).build();
          tableCard.outerHTML = newCard;
          this.showNotification('Fourchette déplacée avec succès', 'success');
        } else {
          this.forcePageRefresh();
        }
      } else {
